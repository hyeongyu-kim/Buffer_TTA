...
def get_accuracy(model: torch.nn.Module,
    ...
    with torch.no_grad():
        for i, data in tqdm(enumerate(data_loader), total = len(data_loader), desc="Evaluating", unit="batch"):
            imgs, labels = data[0], data[1]
            output = model([img.to(device) for img in imgs]) if isinstance(imgs, list) else model(imgs.to(device))
            predictions = output.argmax(1)
            
            # print(imgs.shape,'imgs.shape')
            img_mean = imgs.mean(dim=(1,2,3 ))
            # print(img_mean,'img_mean')
            # print(imgs.mean(0), 'img_mean')
            # print(imgs.mean(0).shape, 'img_mean.shape')
    ...
    # print(f"Evaluation took {t2 - t1:.2f} seconds per batch {i+1} with {num_samples} samples.")
