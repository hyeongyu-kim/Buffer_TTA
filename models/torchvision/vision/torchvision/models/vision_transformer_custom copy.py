...
class FineAffineGateLayerNorm(nn.Module):
    """
    y = ln(x) + α · exp(-(γ·x)²) · x + ε
        ln(x)      : 표준 LayerNorm 결과(learnable scale, bias 포함)
        α, γ (C)   : per-channel residual 규모·gate sharpness
        ε          : 작은 상수, numerically stable
    """
    def __init__(self,
                 hidden_dim: int,
                 eps: float = 1e-6,
        ...
        self.eps    = eps
        # 추가 residual 파라미터
        self.alpha  = nn.Parameter(torch.full((hidden_dim,), init_alpha))
        self.gamma  = nn.Parameter(torch.full((hidden_dim,), init_gamma))

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # ── 1) 표준 LayerNorm ─────────────────────────────────
        mean = x.mean(-1, keepdim=True)
        var  = x.var(-1, unbiased=False, keepdim=True)
        ln   = (x - mean) / torch.sqrt(var + self.eps)
        ln   = ln * self.weight + self.bias

        # ── 2) mid-range residual ────────────────────────────
        mid_gate  = torch.exp(-(self.gamma * x) ** 2)
        residual  = self.alpha * mid_gate * x

        return ln + residual + self.eps
...
class EncoderBlock(nn.Module):
    ...
    def __init__(
        ...
        # Attention block
        if use_tta:
            self.ln_1_tta = norm_layer(hidden_dim) 
        else :
        ...
        if use_tta:
            self.ln_2_tta = norm_layer(hidden_dim) 
        else :
            self.ln_2 = norm_layer(hidden_dim)
    ...
    def forward(self, input: torch.Tensor):
        ...
        x = self.ln_1_tta(input) if self.use_tta else self.ln_1(input)

        x, _ = self.self_attention(x, x, x, need_weights=False)

        # if self.use_tta : 
        #     x = self.transformation_tta(x)

        x = self.dropout(x)
        x = x + input

        # y = self.ln_2(x)
        y = self.ln_2_tta(x) if self.use_tta else self.ln_2(x)
...
class Encoder(nn.Module):
    ...
    def __init__(
        ...
        for i in range(num_layers):
            # use_tta = True if i < 6 else False
            use_tta =  False
            layers[f"encoder_layer_{i}"] = EncoderBlock(
                ...
                attention_dropout,
                # norm_layer,
                norm_layer = partial(FineAffineGateLayerNorm, eps=1e-6) if use_tta else norm_layer,
                use_tta = use_tta
            )
