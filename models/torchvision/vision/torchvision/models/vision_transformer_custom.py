...
class TransformationLayer1D(nn.Module):
    def __init__(self, dim, kernel_sizes=(1,3,5,7)):
        super().__init__()
        self.convs = nn.ModuleList()
        self.scales = nn.ParameterList()
        for k in kernel_sizes:
            pad = (k - 1) // 2
            # embed_dim → embed_dim, local context 살리려면 padding
            conv = nn.Conv1d(dim, dim, kernel_size=k, padding=pad, bias=False)
            self.convs.append(conv)
            # scale 파라미터: 학습 중에 domain shift 보정 강도를 조절
            self.scales.append(nn.Parameter(torch.tensor(0.5)))
    def forward(self, x):
        # x: (B, N, C) → (B, C, N)
        x_t = x.transpose(1, 2)
        # 여러 커널 합성
...
class MLPBlock(MLP):
    ...
    def __init__(self, in_dim: int, mlp_dim: int, dropout: float, activation_layer = nn.GELU):
...
class EncoderBlock(nn.Module):
    ...
    def __init__(
        ...
        if self.use_tta:    
            self.transformation_tta1 = TransformationLayer1D(hidden_dim)
            self.alpha_tta1 = nn.Parameter(torch.tensor(1e-2))

            # self.transformation_tta2 = TransformationLayer1D(hidden_dim)
            # self.alpha_tta2 = nn.Parameter(torch.tensor(1e-2))
            

        # if self.use_tta:
            # self.mlp = MLPBlock(hidden_dim, mlp_dim, dropout, activation_layer = partial(tta_FineAffineGateGELU, mlp_dim))
        # else : 
    ...
    def forward(self, input: torch.Tensor):
        ...
        if self.use_tta:
            x = self.transformation_tta1(x) * self.alpha_tta1 + x
        ...
        # if self.use_tta:
        #     x = self.transformation_tta2(x) * self.alpha_tta2 + x
...
class Encoder(nn.Module):
    ...
    def __init__(
        ...
        for i in range(num_layers):
            use_tta = True if i < 2 else False
            # use_tta = False
            # print(use_tta,'use_tta_in_Encoder')
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        # self.pre_encoder_tta = TransformationLayer1D(self.hidden_dim)
        # 스케일 파라미터: 학습 초반엔 거의 영향 없게 0.1 정도로 초기화
        # self.pre_encoder_alpha = nn.Parameter(torch.tensor(1e-1))
    ...
    def forward(self, x: torch.Tensor):
        ...
        # print('x0')
        ############################################################
        # x = x + self.pre_encoder_alpha * self.pre_encoder_tta(x)
        ############################################################
        # print('x1')
