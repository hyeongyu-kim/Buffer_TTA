...
class Bottleneck(nn.Module):
    ...
    def __init__(
        ...
        if use_tta : 
            self.buffer1 = BufferLayer(width)
            self.alpha_buffer1 = nn.Parameter(torch.tensor(talpha))        
        ...
        if use_tta : 
            ...
            self.buffer2 = BufferLayer(width)
            self.alpha_buffer2 = nn.Parameter(torch.tensor(talpha))
        ...
        if use_tta : 
            ...
            self.buffer3 = BufferLayer(planes * self.expansion)
            self.alpha_buffer3 = nn.Parameter(torch.tensor(talpha))
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.use_tta:
            out = self.buffer3(out) * self.alpha_buffer3 + out
...
class ResNet(nn.Module):
    def __init__(
        ...
        self.buffer0 = BufferLayer(self.inplanes )
        self.alpha_buffer0 = nn.Parameter(torch.tensor(1e-9))
        # self.alpha_tta0 = 0.00005
    ...
    def _make_layer(
        ...
        layers.append(
            block(
                self.inplanes, planes, stride, downsample, self.groups, self.base_width, previous_dilation, norm_layer, use_tta
            )
        )
