...
###################################################################################################################################################
    ...
    # def collect_params(self):
    #     """Only collect from the first 1/3 of all normalization layers."""
    #     # 1) gather all (name, module) pairs that are normalization layers
    ...
    #     # 2) decide how many to keep
    #     cutoff = len(all_norms) // 3

    #     params = []
    #     names = []
    #     # 3) only loop over the first third
    #     for nm, m in all_norms[:cutoff]:
    #         # print("included:", nm)
    ...
    # def configure_model(self):
    #     """Only re‐enable and tweak the first 1/3 of normalization layers."""
    #     self.model.eval()
    #     self.model.requires_grad_(False)

    #     # gather all normalization modules (no names needed here)
    #     norm_types = (nn.BatchNorm1d, nn.BatchNorm2d, nn.LayerNorm, nn.GroupNorm)
    #     all_norms = [m for m in self.model.modules() if isinstance(m, norm_types)]

    #     # compute cut‐off
    #     cutoff = len(all_norms) // 3

    #     # only touch the first third
    #     for m in all_norms[:cutoff]:
    #         # BatchNorm2d: force use batch‐stats and allow grad
    #         if isinstance(m, nn.BatchNorm2d):
    #             m.train()                  # use batch stats even in eval()
    #             m.track_running_stats = False
    #             m.running_mean = None
    #             m.running_var  = None
    #             m.requires_grad_(True)

    #         # BatchNorm1d: train mode avoids single‐sample issues
    #         elif isinstance(m, nn.BatchNorm1d):
    #             m.train()
    #             m.requires_grad_(True)

    #         # LayerNorm & GroupNorm: just turn gradients back on
    #         else:  # LayerNorm or GroupNorm
    #             m.requires_grad_(True)
